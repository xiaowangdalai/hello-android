 ...
 * 3.实现序列化的方法
 * 
 * Android中实现序列化有两个选择：一是实现Serializable接口（是JavaSE本身就支持的），一是实现Parcelable接口（
 * 是Android特有功能
 * ，效率比实现Serializable接口高效，可用于Intent数据传递，也可以用于进程间通信（IPC））。实现Serializable接口非常简单
 * ，声明一下就可以了，而实现Parcelable接口稍微复杂一些，但效率更高，推荐用这种方法提高性能。
 * 
 * 注：Android中Intent传递对象有两种方法：一是Bundle.putSerializable(Key，Object)，另一种是Bundle.
 * putParcelable
 * (Key，Object)。当然这些Object是有一定的条件的，前者是实现了Serializable接口，而后者是实现了Parcelable接口。
 * 
 * 4.选择序列化方法的原则
 * 
 ...
 * 6、Parcelable接口定义
 * 
 * 复制代码 public interface Parcelable { //内容描述接口，基本不用管 public int
 * describeContents(); //写入接口函数，打包 public void writeToParcel(Parcel dest, int
 * flags); //读取接口，目的是要从Parcel中构造一个实现了Parcelable的类的实例处理。因为实现类在这里还是不可知的，
 ...
 * 也可以将Parcel看成是一个流
 * ，通过writeToParcel把对象写到流里面，在通过createFromParcel从流里读取对象，只不过这个过程需要你来实现
 * ，因此写的顺序和读的顺序必须一致。
 * 
 * 代码如下：
 * 
 ...
 * 
 * 2）Parcelabel的实现，不仅需要implements Parcelabel，还需要在类中添加一个静态成员变量CREATOR，这个变量需要实现
 ...
 * 
 * 复制代码 public class Person implements Serializable { private static final long
 * serialVersionUID = -7060210544600464481L; private String name; private int
 * age;
 * 
 ...
 *           out.writeString(bookName); out.writeString(author);
 ...
 * 
 *           public Book(Parcel in) { bookName = in.readString(); author =
 *           in.readString(); publishDate = in.readInt(); } } 复制代码 分类:
 *           Android基础知识
 */
